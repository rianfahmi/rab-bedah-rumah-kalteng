...
export function rabExport(record,doc){
 ...
 const rows=assessment.items.map((r,i)=>[record.id,record.year,record.name,record.region,record.district,record.village,doc.activity,r.template===true?r.no:(r.no??i+1),r.description,r.unit,r.volume,r.price,r.cost,r.stage1,r.stage2,r.cash,r.reused,r.section?null:Math.round((r.cost-r.allocated)*100)/100]);
