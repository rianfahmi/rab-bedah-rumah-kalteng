...
export async function openRab(record){
 const pageMode=document.documentElement.classList.contains('document-page');
 const dialog=document.getElementById('document'),root=pageMode?document.getElementById('app'):document.getElementById('doc-content');
 const closeDocument=()=>{if(pageMode)location.hash='#bnba/'+record.year+'/'+record.id;else dialog.close()};
 if(!pageMode){dialog.classList.add('rab-dialog');dialog.addEventListener('close',()=>{dialog.classList.remove('rab-dialog');dialog.oncancel=null;},{once:true});}
 root.innerHTML=pageMode?'<main class="document-route rab-route"><div class="modal-body">Memuat isian RAB…</div></main>':'<div class="modal-body">Memuat isian RAB…</div>';
 ...
 try{
  ...
  root.innerHTML=(pageMode?'<main class="document-route rab-route">':'')+`<div class="modal-body">${esc(error.message)}<button id="rab-close">Tutup</button></div>`+(pageMode?'</main>':'');
  root.querySelector('button').onclick=closeDocument;
  return;
 }
 ...
 const exportButton=root.querySelector('#rab-export'),editButton=form.querySelector('[type="submit"]'),addButton=root.querySelector('#rab-add'),closeButton=root.querySelector('#rab-close'),cancelButton=root.querySelector('#rab-cancel');
 ...
 function setEditMode(enabled){
  ...
  const cancelLabel=enabled?'Batalkan perubahan':'Tutup RAB';
  closeButton.setAttribute('aria-label',cancelLabel);cancelButton.setAttribute('aria-label',cancelLabel);
 }
 function renderRows(){
 ...
 const cancelOrClose=()=>{if(editing){if(pageMode)openRab(record);else{dialog.close();setTimeout(()=>openRab(record),0)}return}closeDocument();};
 closeButton.onclick=cancelOrClose;cancelButton.onclick=cancelOrClose;
 if(!pageMode)dialog.oncancel=event=>{if(editing){event.preventDefault();cancelOrClose();}};
